	...
	<section class="hbox stretch">
		<section>
			...
			<div class="alert alert-success alert-dismissable"
						...
						<strong>Success!</strong>Report File Uploaded Successfully.
					</div>	
                          <table class="table table-bordered table-striped table-responsive" id="example">
                              <thead>
                               <tr><th style="text-align: center;">ReqId</th>
...
window.onload = function() {
	...
	$.ajax({
		url : "rest/cs/get/allBankStatementRequest",
		type : 'GET',
		dataType : "json",
		...
		complete : function(data, textStatus, $XHR) {
			...
			if ((data.resp_code) == 'CS1002') {
	            ...
	            $.each(data.data, function(key, requestData) {
	                ...
	                console.log("requestData"+requestData.reportFile);
	                var a=requestData.reportFile;
	                
	               
	                if( a=='Not Present'){
	                	$("#requestMap").append("<tr><td style='text-align:center;'>"+requestData.id+"</td><td style='text-align:center;'>"+requestData.userName+"</td><td style='text-align:center;'>"+requestData.bankName+"</td><td style='text-align:center;'>"+requestData.creationTime+"</td><td style='text-align:center;'><a href='rest/cs/download/bankFile/"+requestData.bankFile+"/"+requestData.userName+"'>"+requestData.bankFile+"</a></td><td style='text-align:center;'><p id='msg"+i+"' style='color:green;'>File Uploaded Successfully</p><input type='file'  accept='application/pdf,application/vnd.ms-excel' id='reportFile"+i+"' class='file' onchange= uploadReportFile('"+requestData.id+"','"+requestData.userName+"','"+this.id+"') /></td></tr>");
	                	 findlength("msg"+i);
	            	       i++; 
		                }
	                else{
	                	  $("#requestMap").append("<tr><td style='text-align:center;'>"+requestData.id+"</td><td style='text-align:center;'>"+requestData.userName+"</td><td style='text-align:center;'>"+requestData.bankName+"</td><td style='text-align:center;'>"+requestData.creationTime+"</td><td style='text-align:center;'><a href='rest/cs/download/bankFile/"+requestData.bankFile+"/"+requestData.userName+"'>'"+requestData.bankFile+"</a></td><td><a href='rest/cs/download/reportFile/"+requestData.reportFile+"/"+requestData.userName+"'>"+requestData.reportFile+" <p id='msg"+i+"' style='color:green;'>File Uploaded Successfully</p><label> <i class='fa fa-cloud-upload'></i>Upload <input type='file' class='file'  style='display:none;' id='reportFile"+i+"'  onchange= uploadReportFile('"+requestData.id+"','"+requestData.userName+"','"+this.id+"') /></label></td></tr>");
	                	  findlength("msg"+i);
		                	  i++;
	                }
...
function uploadReportFile(reqId,userName,id){
	var dataObj=new FormData();
// 	$.each($('#reportFile')[0].files, function(i, file) {
// 		dataObj.append('file', file);
// 	});
	$.each($("#reportFile"+id)[0].files, function(i, file) {
		dataObj.append('file', file);
	});

	$.LoadingOverlay("show");
	$.ajax({
		url : "rest/cs/uploadReportFile/"+reqId+"/"+userName,
		type : 'POST',
		data : dataObj,
  contentType: false,
  processData: false,
