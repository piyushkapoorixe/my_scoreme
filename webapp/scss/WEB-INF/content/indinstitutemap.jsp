				...
				<section class="scrollable padder">
					<section id="content" class="m-t-lg wrapper-md animated fadeInDown">
						<div class="container aside-xl" style="border: 1px solid #ccc;margin-bottom:10px;">
							<h3 class="head-txt">Industry Institute Mapping</h3>
							<section class="m-b-lg">
								<form id="instituteMapping-form" data-validate="parsley" action="#">
								 <div class="alert alert-success" id="mapped-success"
						            style="display: none;">
					       	        <i class="fa fa-ok-sign"></i> Mapped SuccessFully.
				                </div>
				                
			                	 <div class="alert alert-danger" id="mapped-unsuccess"
						            style="display: none;">
					       	        <i class="fa fa-ok-sign"></i>Unable to Map.
				                </div>
				                
									<div class="list-group">
                                     <span>Business Type <b style="color: red;">*</b></span>
									 <div class="list-group-item">
										<select name="businessType" id="businessType" ng-model="businessType"
											data-required="true" title="Please select the username"
											class="form-control no-border" onchange="getIndustry()">
											<option value="">Select Business </option>		
											<option ng-repeat="i in businessTypeAdminData.data" value="{{i.id}}">{{i.groupName}}</option>	
										</select>
									</div>
									
									 <span>Industry Type <b style="color: red;">*</b></span>
									 <div class="list-group-item">
										<select name="industryType" id="industryType" ng-model="industryType"
											data-required="true" title="Please select the username" 
											class="form-control no-border js-example-responsive" multiple="multiple">
										</select>
									</div>
									
									<span>Institute Name <b style="color: red;">*</b></span>
									 <div class="list-group-item">
										<select name="instituteName" id="instituteName" ng-model="instituteName"
											data-required="true" title="Please select the institute" ng-controller="instituteController"
									...
									<span>Report Type <b style="color: red;">*</b></span>
									 <div class="list-group-item">
										<select name="reportType" id="reportType" ng-model="reportType"
											data-required="true" title="Please select the reportType"
											class="form-control no-border">
										</select>
									</div>
								</div>
									...
									<button type="button" 
										class="btn btn-lg btn-primary btn-block"
										 id="validate-button" onclick="mapIndustryInstitute()"
...
window.onload=function(){
	...
	/* $.ajax({
		url : "rest/cs/get/BusinessTypeAdmin",
		type : 'GET',
		dataType : "json",
		contentType : "application/json; charset=utf-8",
		complete : function(data, textStatus, $XHR) {
	...
	$.ajax({
		url : "rest/cs/get/industryInstituteMapData",
		type : 'GET',
		dataType : "json",
		contentType : "application/json; charset=utf-8",
		complete : function(data, textStatus, $XHR) {
			data = data["responseJSON"];
			$("#industryMapTable").html("");
			if ((data.resp_code) == 'CS1002') {
				console.log(data.data);
	            $.each(data.data, function(key, industryInst) {
	               $("#industryMapTable").append("<tr><td style='text-align:center;'>"+industryInst.industryName+"</td><td style='text-align:center;'>"+industryInst.instituteName+"</td></tr>");
	            });
	            $("#example").DataTable();
	        } else {
	            console.log(data.resp_code);
...
function getIndustry(){
		...
		var url = "rest/cs/get/getAllIndutsryAdmin/"+ groupCode;
		$.ajax({
			url:url,
		    type:'GET',
		    dataType : "json",
		...
		$.ajax({
			url : "rest/cs/get/reports/"+groupCode,
			type : 'GET',
			dataType : "json",
			contentType : "application/json; charset=utf-8",
			complete : function(data, textStatus, $XHR) {
				...
				if ((data.resp_code) == 'CS1002') {
					...
					$('#reportType').html("");
		            $('#reportType').append('<option value="">SELECT REPORT</option>');
					//jQuery('#company-form select[id="businessType"] option:gt(0)').remove().end();
		            $.each(data.data, function(key, report) {
		                $('#reportType').append('<option value="' + report.id + '">' + report.reportName + '</option>');
		            });
		        } else {
		            console.log(data.resp_code);
		        }
...
function mapIndustryInstitute(){

	//alert($("#industryType").val());

		var jsonData = {
		   industryList : $("#industryType").val(),
			instituteId:$("#instituteName").val(),
			industrygroupId:$("#businessType").val(),
		reportId:$("#reportType").val()
		};

		$.ajax({
			url : "rest/cs/add/industryInstitute",
			type : 'POST',
			dataType : "json",
			contentType : "application/json; charset=utf-8",
			...
			complete : function(data, textStatus, $XHR) {
				...
				if (data["resp_code"] == "CS1002") {
					...
					$("#reportType").val('');
					$("#mapped-success").show();
				    $("#mapped-success").delay(2200).fadeOut(500);
				} else {
					//$("#request-failure").show();
					$("#mapped-unsuccess").show();
					 $("#mapped-unsuccess").delay(2200).fadeOut(500);
				}
			}
		});
}
