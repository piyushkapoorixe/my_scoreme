<section id="content">
    ...
    <div class="panel panel-primary bankDetailForm">
        <div class="panel-heading" style="margin-top:10px;">
            <h3 class="panel-title text-center">Download Documents</h3>
        </div>
...
<!--                                   </div> -->
            ...
            <form name="submitCredentialForm" method="post" enctype="multipart/form-data" >
                <div class="col-md-4">
                  <div class="form-group ">
                     ...
                     <div class="input-group">
                          <select name="userName" id="userName" class="form-control" onchange="getUserCompanies()" required >
                          </select>
                     </div>
                 </div>
               </div>
               ...
               <div class="col-md-4">
                 <div class="form-group ">
                    <p class="control-label">Company Name</p>
                    <div class="input-group">
                         <select name="companyName" id="companyName" class="form-control" onchange="getUserCompanyRatings()" required >
                         </select>
                    </div>
                </div>
              </div>
              ...
              <div class="col-md-4">
                <div class="form-group ">
                   <p class="control-label">RatingId-RatingDateTime</p>
                   <div class="input-group">
                        <select name="ratingId" id="ratingId" class="form-control" onchange="getDocumentsByRatingId()" required >
                        </select>
                   </div>
               </div>
             </div>
					...
					<div class="col-lg-12 table-responsive">
						<div class="col-lg-12 table-responsive">
							<table
								...
								<thead>
									<tr>
										<th style="text-align: center;">Rating Id</th>
										<th style="text-align: center;">Balance Sheet</th>
										<th style="text-align: center;">Profit and Loss</th>
										<th style="text-align: center;">Bank Statement</th>
...
function getUserCompanies(){
	...
	$.ajax({
		url : "rest/cs/get/userCompanies/"+userId,
		type : 'GET',
		dataType : "json",
		contentType : "application/json; charset=utf-8",
		complete : function(data, textStatus, $XHR) {
			...
			if ((data.resp_code) == 'CS1002') {
	            $('#companyName').html("");
	            $('#companyName').append('<option value="">SELECT Company</option>');
	            $.each(data.data, function(key, company) {
	                $('#companyName').append('<option value="' + company.id + '">' + company.companyName+'</option>');
	            });
	           // $("#example").DataTable();
	        } else {
	            console.log(data.resp_code);
...
function getUserCompanyRatings(){
	$('#ratingMap').html("");
	var userId=$("#userName").val();
	var companyId=$("#companyName").val();
	$.ajax({
		url : "rest/cs/get/userCompanyRatings/"+userId+"/"+companyId,
		type : 'GET',
		dataType : "json",
		contentType : "application/json; charset=utf-8",
		complete : function(data, textStatus, $XHR) {
			...
			if ((data.resp_code) == 'CS1002') {
	            $('#ratingId').html("");
	            $('#ratingId').append('<option value="">SELECT Rating</option>');
	            $.each(data.data, function(key, rating) {
	            	$('#ratingId').append('<option value="' + rating.id + '">' + rating.id+'-  '+rating.creationTime+'</option>');
	                //;
...
function getDocumentsByRatingId(){
	var ratingId=$("#ratingId").val();
	$.ajax({
		url : "rest/cs/get/documentsByRatingId/"+ratingId,
		type : 'GET',
		dataType : "json",
		contentType : "application/json; charset=utf-8",
		complete : function(data, textStatus, $XHR) {
			...
			if ((data.resp_code) == 'CS1002') {
	            $('#ratingMap').html("");
	            //$('#ratingId').append('<option value="">SELECT Rating</option>');
	            $.each(data.data, function(key, upload) {
	            	$('#ratingMap').append("<tr><td style='text-align:center;'>"+upload.ratingId+"</td><td style='text-align:center;'><a href='rest/cs/download/uploadDocuments/"+upload.balancesheet+"/"+upload.userName+"'>"+upload.balancesheet+"</a></td><td style='text-align:center;'><a href='rest/cs/download/uploadDocuments/"+upload.profitandloss+"/"+upload.userName+"'>"+upload.profitandloss+"</a></td><td style='text-align:center;'><a href='rest/cs/download/uploadDocuments/"+upload.bankstatement+"/"+upload.userName+"'>"+upload.bankstatement+"</a></td><td>"+upload.statementFrom+"</td><td>"+upload.statementTo+"</td></tr>");
	                //;
