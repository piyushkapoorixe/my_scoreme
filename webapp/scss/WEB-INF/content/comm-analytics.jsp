		...
		<section class="scrollable padder">
			...
			<section class="panel panel-default">
				...
				<footer class="panel-footer bg-white">
					<div class="row text-center no-gutter">
						<div class="col-xs-4 b-r b-light">
							<p id="fsales" class="h3 font-bold m-t">${commBean.getValue('OP_INC')}</p>
							<p class="text-muted">Operating Income</p>
						</div>
						<div class="col-xs-4 b-r b-light">
							<p id="fprofits" class="h3 font-bold m-t">${commBean.getValue('PAT')}</p>
							<p class="text-muted">Profit After Tax</p>
						</div>
						<div class="col-xs-4 b-r b-light">
							<p id="fpbt" class="h3 font-bold m-t">${commBean.getValue('TDEBT')}</p>
			...
			<section class="panel panel-default">
				...
				<footer class="panel-footer bg-white">
					<div class="row text-center no-gutter">
						<div class="col-xs-3 b-r b-light">
							<p id="fsales" class="h3 font-bold m-t">${commBean.getValue('OP_PRO_MAR')}</p>
							<p class="text-muted">Operating Profit Margin</p>
						</div>
						<div class="col-xs-3 b-r b-light">
							<p id="fprofits" class="h3 font-bold m-t">${commBean.getValue('NET_PRO_MAR')}</p>
							<p class="text-muted">Net Profit Margin</p>
						</div>
						<div class="col-xs-3 b-r b-light">
							<p id="fpbt" class="h3 font-bold m-t">${commBean.getValue('RET_CAP')}</p>
							<p class="text-muted">Return on Capital Employed</p>
						</div>
						<div class="col-xs-3 b-r b-light">
							<p id="fpbt" class="h3 font-bold m-t">${commBean.getValue('NET_CASH')}</p>
...
<script type="text/javascript">
               ...
               var f4 = [], f5 = [], f6 = [], f7 = [];
               <c:forEach items="${commBean.getLongValue('OP_INC')}" var="net1" varStatus="mb1">
               sales.push([index1++, ${net1}]);
               </c:forEach>
               var index2 = 1;
               <c:forEach items="${commBean.getLongValue('PAT')}" var="net2" varStatus="mb2">
               profits.push([index2++, ${net2}]);
               </c:forEach>
               var index3 = 1;
               <c:forEach items="${commBean.getLongValue('TDEBT')}" var="net3" varStatus="mb3">
               pbt.push([index3++, ${net3}]);
               </c:forEach>
               var index4 = 1;
               <c:forEach items="${commBean.getLongValue('OP_PRO_MAR')}" var="net3" varStatus="mb3">
               f4.push([index4++, ${net3}]);
               </c:forEach>
               var index5 = 1;
               <c:forEach items="${commBean.getLongValue('NET_PRO_MAR')}" var="net3" varStatus="mb3">
               f5.push([index5++, ${net3}]);
               </c:forEach>
               var index6 = 1;
               <c:forEach items="${commBean.getLongValue('RET_CAP')}" var="net3" varStatus="mb3">
               f6.push([index6++, ${net3}]);
               </c:forEach>
               var index7 = 1;
               <c:forEach items="${commBean.getLongValue('NET_CASH')}" var="net3" varStatus="mb3">
               f7.push([index7++, ${net3}]);
               </c:forEach>
               ...
               <c:forEach items="${commBean.getKeyValue('YEXIST')}" var="nf1" >
               yexist.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('AVG_TOFF')}" var="nf1" >
               offtake.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('TYPE_PROJ')}" var="nf1" >
               typproj.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('AVG_BOOK')}" var="nf1" >
               avgbook.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('TECH_OP')}" var="nf1" >
               tech.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('DIV_GEO')}" var="nf1" >
               divgeo.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('PRO_MARGIN')}" var="nf1" >
               promar.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('NO_PROJ')}" var="nf1" >
               noproj.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('NO_ONG')}" var="nf1" >
               noplan.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('EXP_APP')}" var="nf1" >
               expapp.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('CASF_FLOW')}" var="nf1" >
               cashflow.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('FUNDR')}" var="nf1" >
               fundr.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('SS_PASPROJ')}" var="nf1" >
               sspp.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('LAND_AVA')}" var="nf1" >
               lavail.push({label: '${nf1.key}', data: ${nf1.value}});
               </c:forEach>
               <c:forEach items="${commBean.getKeyValue('FSI_RULE')}" var="nf1" >
