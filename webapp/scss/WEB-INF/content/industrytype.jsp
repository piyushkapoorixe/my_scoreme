		...
		<section>
				...
				<section class="scrollable padder">
					<section id="content" class="m-t-lg wrapper-md animated fadeInDown">
						<div class="container aside-xl" style="border: 1px solid #ccc;margin-bottom:10px;">
							<h3 class="head-txt">Industry Type</h3>
							<section class="m-b-lg">
								<form id="industrytype-form" data-validate="parsley" action="#">
							      <div class="alert alert-success alert-dismissable" id="save-industrytype" style="display: none;width:353px;margin-left: -13px;">
                                    <strong>Success!</strong>Industry Type Saved successfully.
                                  </div>
                   
                                  <div class="alert alert-danger alert-dismissable" id="industrytype-failure" style="display: none;width: 353px;margin-left: -13px;">
                                    <strong>Failure!</strong>Unable To Save Industry Type.
                                  </div>

									<div class="list-group">
									    <div class="list-group-item">
										  <select name="businessType" id="businessType" data-required="true"
											 title="Please select the Business"
												class="form-control no-border" ng-model="businessType">
									...
									<div class="list-group">
										<div class="list-group-item">
											<input type="text" name="industryType" id="industryType"
												class="form-control no-border" 
												text="Please enter Industry type"
												placeholder="Enter Industry Type" data-required="true" ng-model="industryType">
										</div>
									</div>
									
									<button type="button" 
										class="btn btn-lg btn-primary btn-block"
										ng-click="addIndustryType()" id="addindustry-button"
			...
			<div class="col-lg-12 table-responsive">		
                          <table class="table table-bordered table-striped table-responsive" id="example">
                              <thead>
                               <tr><th style="text-align: center;">Nature Of Business</th>
                               <th style="text-align: center;">Industry </th>
                               </tr>
                             </thead>
                          <tbody id="businessIndustryMap">
                                <tr ng-repeat="i in buisnessIndustryTypeData.data">
                             <td style='text-align:center;'>{{i.groupName}}</td> 
                             <td style='text-align:center;'>{{i.industryName}}</td>  
                            </tr>  
                          </tbody>
                         </table>
                      </div> 		
...
/* window.onload = function() { */
	...
	$("#industryType").keypress(function(event){
	...
	/* $.ajax({
		url : "rest/cs/get/businessIndustryType",
		type : 'GET',
		dataType : "json",
			...
			if ((data.resp_code) == 'CS1002') {
	            ...
	            $.each(data.data, function(key, industrygroup) {
	                //$('#businessType').append('<option value="' + industrygroup.id + '">' + industrygroup.groupName + '</option>');
	            	 $("#businessIndustryMap").append("<tr><td style='text-align:center;'>"+industrygroup.groupName+"</td><td style='text-align:center;'>"+industrygroup.industryName+"</td></tr>");
	            });
	            $("#example").DataTable();
	        } else {
	        	// jQuery('#industrytype-form select[id="businessType"] option:gt(0)').remove().end();
...
/* function addIndustryType(){
	  if ($("#industrytype-form").parsley('validate')) {

		  var jsonData={
				industryGroupId:$("#businessType").val(),
				industryName:$("#industryType").val()  
		  }
		  $.ajax({
				url : "rest/cs/createIndustryType",
				type : 'POST',
				dataType : "json",
				contentType : "application/json; charset=utf-8",
				...
				complete : function(data, textStatus, $XHR) {	
					...
					if (data["resp_code"] == "CS1002"){
						...
						$("#industryType").val('');
						$("#businessType").val('');
						 $("#save-industrytype").show();
						 $("#save-industrytype").delay(2200).fadeOut(500);
						
					}
					else{
						$("#industrytype-failure").show();
						$("#industrytype-failure").delay(2200).fadeOut(500);
