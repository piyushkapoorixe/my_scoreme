...
<section id="content" ng-controller="loginController">
				...
				<section class="scrollable padder">
					<section id="content" class="m-t-lg wrapper-md animated fadeInDown">
						<div class="container aside-xl" style="border: 1px solid #ccc;margin-bottom:10px;">
							<h3 class="head-txt">Mobile  Verification</h3>
							<section class="m-b-lg">
								<form id="verifymobile-form" data-validate="parsley" action="#">
								 <div class="alert alert-success" id="verify-OTP"
						            style="display: none;">
					       	        <i class="fa fa-ok-sign"></i> Verified SuccessFully.
				                </div>
				                
			                	 <div class="alert alert-danger" id="verify-Mobile"
						            style="display: none;">
					       	        <i class="fa fa-ok-sign"></i> Enter 10 Digit Mobile No.
				                </div>
				                
				                 <div class="alert alert-danger" id="verify-NOTOTP"
						            style="display: none;">
					       	        <i class="fa fa-ok-sign"></i>Please Check OTP And Mobile No. Again
				                </div>
									<div class="list-group">
										<div class="list-group-item">
											<input type="text" name="mobileNo" id="mobileNo" ng-model="mobileNo"
												class="form-control no-border" maxlength="10"
												text="Please enter Mobile No."
												placeholder="Enter Mobile No." data-required="true"  data-parsley-type="number" onkeypress="return isNumber(event)">
												<i class="fa fa-refresh" aria-hidden="true" id="reload"></i>
										</div>
									<input type="hidden" value="${requestScope.pageVerify}" id="pageMobile">	
									<input type="hidden" value="${requestScope.pageVerifyAssocham}" id="pageAssocham">
									<input type="hidden" id="pageVerifyData" name="pageVerifyData">
									</div>
									<button type="button" 
										class="btn btn-lg btn-primary btn-block"
										 id="generate-button" ng-click="generateOTP()"
							     	...
							     	<div class="list-group">
										<div class="list-group-item">
											<input type="text" name="otp" id="otp" ng-model="otp"
												class="form-control no-border" maxlength="6"
												text="Please enter OTP"
												placeholder="Enter OTP" data-required="true"  data-parsley-type="number" onkeypress="return isNumber(event)">
										</div>
									</div>
									...
									<button type="button" disabled
										class="btn btn-lg btn-primary btn-block"
										 id="validate-button" ng-click="validateOTP()"
										style="background-color: #FAA61A; border-color: #FAA61A; border-radius: 0px;">
									   Validate</button>
...
$(document).ready(function(){
	if($("#pageAssocham").val()==151){
		$("#divHideLogo").removeClass("hide-logo");
		$("#iconbarMenu").hide();
	}
});

window.onload=function(){
	$("#menu-toggle").trigger("cilck");
...
/* function generateOTP()
{
	 if($("#mobileNo").val().length==10){
         $.ajax({
            url:"rest/cs/get/OTP/"+$("#mobileNo").val(),
      	    type : 'GET',
			dataType : "json",
		   ...
		   $("#verify-Mobile").show();
		   $("#verify-Mobile").delay(2200).fadeOut(500);
		   console.log("Enter 10 Digit Mobile No.");
	   }
}
 */
...
// 				};
		   $.ajax({
	            url:"rest/cs/get/validateOTP/"+$("#mobileNo").val()+"/"+$("#otp").val(),
	      	    type : 'GET',
				dataType : "json",
				contentType : "application/json; charset=utf-8",
				complete : function(data, textStatus, $XHR) {
					...
					if ((data.resp_code) == 'CS1002') {
						...
						$("#generate-button").prop('disabed',true);
						$("#validate-button").prop('disabed',true);
						$("#pageVerifyData").val('verify');
						//var verified="verify";
						console.log("verify"+$("#pageVerifyData").val());
						callApp("page,dashboard,"+$("#pageVerifyData").val()+",1");
						console.log("after call");
					    console.log(data.resp_code);
			        } else {
				        $("#verify-NOTOTP").show();
				        $("#verify-NOTOTP").delay(2200).fadeOut(500);
			            console.log(data.resp_code);
			        }
				}
	         });
