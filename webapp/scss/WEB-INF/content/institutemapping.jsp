<section id="content" ng-controller="instituteController">
				...
				<section class="scrollable padder">
					<section id="content" class="m-t-lg wrapper-md animated fadeInDown">
						<div class="container aside-xl" style="border: 1px solid #ccc;margin-bottom:10px;">
							<h3 class="head-txt">User Institute Mapping</h3>
							<section class="m-b-lg">
								<form id="instituteMapping-form" data-validate="parsley" action="#">
								 <div class="alert alert-success" id="successuser-institutemap"
						            style="display: none;">
					       	        <i class="fa fa-ok-sign"></i> Mapped Successfully.
				                </div>
				                
			                	 <div class="alert alert-danger" id="unsuccess-institutemap"
						            style="display: none;">
					       	        <i class="fa fa-ok-sign"></i>Unable To Map
				                </div>
									...
									<div class="list-group">
									 ...
									 <div class="list-group-item">
										<select name="userName" id="userName"
											data-required="true" title="Please select the username"
											class="form-control no-border" ng-model="userName">
										<option value="">SELECT USER NAME</option>
										<option ng-repeat="u in userData.data" value="{{u.id}}">{{u.firstName}}-{{u.username}}</option>
										</select>
									</div>
									
									<span>Institute Name <b style="color: red;">*</b></span>
									 <div class="list-group-item">
										<select name="instituteName" id="instituteName"
											data-required="true" title="Please select the institute"
											class="form-control no-border" ng-model="instituteName">
											<option value="">SELECT INSTITUTE NAME</option>
											<option ng-repeat="i in instituteData.data" value="{{i.id}}">{{i.instituteName}}</option>
								
										</select>
									</div>
									<input type="hidden" id="savevar">
								</div>
									
									<button type="button" 
										class="btn btn-lg btn-primary btn-block"
										 id="validate-button" ng-click="mapUserInstitute()"
								...
								data-target="#myModalHorizontal">Multiple User Map</button>
								<button class="btn btn-primary btn-s-xs"><a href="rest/cs/download/userTemplate/xls" style="color: white;">User Excel Template</a></button>
							</section>
						</div>
					</section>
					
		<div class="col-lg-12 table-responsive">		
			<div class="col-lg-12 table-responsive" style="height: 500px;">		
                          ...
                          <tbody id="userMapTable">
                               <tr ng-repeat="um in userMapData.data">
                               <td style="text-align: center;">{{um.firstName}}</td>
                               <td style="text-align: center;">{{um.username}}</td>
                               <td style="text-align: center;">{{um.instituteName}}</td>
                               </tr>
                          </tbody>
                         </table>
                      </div> 		
						...
						<div class="modal-dialog">
							<div class="modal-content"
								...
								<div class="modal-header">
									...
									<h4 class="modal-title" id="myModalLabel" style="margin-top:25px">Upload Files</h4>
								</div>
								...
								<div class="modal-body">
									<div class="alert alert-success" id="save-upload-success"
										style="display: none;">
										<button type="button" class="close" onclick="hidediv(this);" style="margin-top: 25px;">x</button>
										...
										<div class="form-group">
											...
											<div class="col-md-4">
												<input type="file" id="file1" class="form-control"
													 placeholder="UserSheet" required/>
											</div>
												<label class="col-sm-2 control-label">Institute
													Name</label>
												<div class="col-md-4">
													<select name="instituteName" id="instituteNameAjax"
														required title="Please select the institute" class="form-control" required="true">
														<option value="">SELECT INSTITUTE NAME</option>
											            <option ng-repeat="i in instituteData.data" value="{{i.id}}">{{i.instituteName}}</option>
													</select>
												</div>
										...
										<div class="form-group">
											<div class="col-sm-offset-2 col-sm-10">
												<button type="button" class="btn btn-default"
													onclick="multipleUserMap()">Upload</button>
											</div>
										</div>
										<div class="row">
										   <div class="col-lg-12 col-sm-8">
										      <table class="table table-bordered table-striped table-responsive">
										         <thead>
										            <tr><th>Username</th><th>Mapped</th></tr>
										         </thead>
										         <tbody id="unsuccess-userMap">
										         </tbody>
										      </table>
										   </div>
										</div>
...
window.onload=function(){
	...
	if(sessionStorage.getItem("saveSuccess")==1){
		$("#successuser-institutemap").show();
	    $("#successuser-institutemap").delay(2200).fadeOut(500);
	}


	/* $.ajax({
		url : "rest/cs/get/users",
		type : 'GET',
		dataType : "json",
		...
		complete : function(data, textStatus, $XHR) {
			...
			if ((data.resp_code) == 'CS1002') {
	            ...
	            $.each(data.data, function(key, user) {
	                $('#userName').append('<option value="' + user.id + '">' + user.firstName+" "+"-"+" "+user.username + '</option>');
	            });
	           // $("#example").DataTable();
	        } else {
	...
	/* $.ajax({
		...
		complete : function(data, textStatus, $XHR) {
			...
			if ((data.resp_code) == 'CS1002') {
	            $('#instituteName').html("");
	            $('#instituteNameAjax').html("");
	            $('#instituteName').append('<option value="">SELECT INSTITUTE</option>');
	            $('#instituteNameAjax').append('<option value="">SELECT INSTITUTE</option>');
	            $.each(data.data, function(key, institute) {
	                $('#instituteName').append('<option value="' + institute.id + '">' + institute.instituteName + '</option>');
	                $('#instituteNameAjax').append('<option value="' + institute.id + '">' + institute.instituteName + '</option>');
	            });
	        } else {
	            console.log(data.resp_code);
	        }
 ...
 */
	...
	/* $.ajax({
		url : "rest/cs/get/userInstituteMapData",
		type : 'GET',
		dataType : "json",
		contentType : "application/json; charset=utf-8",
		complete : function(data, textStatus, $XHR) {
			...
			if ((data.resp_code) == 'CS1002') {
	            ...
	            $.each(data.data, function(key, user) {
	               $("#userMapTable").append("<tr><td style='text-align:center;'>"+user.firstName+"</td><td style='text-align:center;'>"+user.username+"</td><td style='text-align:center;'>"+user.instituteName+"</td></tr>");
	            });
	            $("#example").DataTable();
	        } else {
	            console.log(data.resp_code);
		...
		$.ajax({
			...
			complete : function(data, textStatus, $XHR) {
				...
				if (data["resp_code"] == "CS1002") {
					...
					console.log("success");
					$("#savevar").val(1);
					sessionStorage.setItem("saveSuccess", 1);
					location.reload(true);
				} else {
					$("#unsuccess-institutemap").show();
					$("#userName").val('');
					$("#instituteName").val('');
					 $("#unsuccess-institutemap").delay(2200).fadeOut(500);
				}
			}
		});
 ...
 */
 ...
 function multipleUserMap(){
	...
	if($("#instituteNameAjax").val()!="" && $("#file").val()!=""){
	var dataObj=new FormData();

	$.each($('#file1')[0].files, function(i, file) {
		dataObj.append('file', file);
	});
	dataObj.append('instituteName',$("#instituteNameAjax").val());
	$.ajax({
		type:'POST',
		...
		success : function(data, textStatus, $XHR) {
			if(data.length>0){
			  ...
			  var content=newData.split(",");
			  $("#unsuccess-userMap").html("");
			  for(var i=0;i<content.length;i++){
				 $("#unsuccess-userMap").append("<tr><td>"+content[i]+"</td><td>No</td></tr>");
			 }
		}else{
